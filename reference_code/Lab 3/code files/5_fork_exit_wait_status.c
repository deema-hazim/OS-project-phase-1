#include <stdio.h>    // puts(), printf(), perror(), getchar()
#include <stdlib.h>   // exit(), EXIT_SUCCESS, EXIT_FAILURE
#include <unistd.h>   // getpid(), getppid(),fork()
#include <sys/wait.h> // wait()


//Notes about exit status codes: 

// Few exit status codes are listed below for extra information :

// 1 : Miscellaneous errors, such as “divide by zero” and other impermissible operations.
// 2 : Missing keyword or command, or permission problem.
// 126 : Permission problem or command is not an executable
// 128 : invalid argument to exit.
// Note : The C standard does not define the meaning of return codes. 
// Rules for the use of return codes vary on different platforms.


void child() {
  int status = 42; //any non zero value

  printf(" CHILD <%ld> I'm alive and my PPID = %ld.\n",
         (long) getpid(), (long) getppid());

  printf(" CHILD <%ld> Goodbye, exit with status %d.\n",
         (long) getpid(), status);

  exit(status);
}

void parent(pid_t pid) {
  int status;

  printf("PARENT <%ld> Spawned a child with PID = %ld.\n",
         (long) getpid(), (long) pid);

  //wait(&status): passing a pointer to the int status
  //can also do pid = wait(&status) to return the pid of the terminated child process
  //pid_t pid_terminated_child = wait(&status);
  wait(&status);

  printf("PARENT <%ld> had a terminated child which had a PID = %ld.\n",
    (long) getpid(), (long) pid);


  //WIFEXITED: returns true if the child terminated normally (not because of some signal)

  //WEXITSTATUS: returns the exit status of the child.
  // This should be employed only if WIFEXITED returned true.
  if (WIFEXITED(status)) {
    printf("PARENT <%ld> Child with PID = %ld and exit status = %d terminated.\n",
           (long) getpid(), (long) pid, WEXITSTATUS(status));
  }

  printf("PARENT <%ld> Goodbye!\n",
         (long) getpid());

  exit(EXIT_SUCCESS);
}

int main(void) {

  pid_t pid;

  switch (pid = fork()) {
  case -1:
    // On error fork() returns -1.
    perror("fork failed");
    exit(EXIT_FAILURE);
  case 0:
    // On success fork() returns 0 in the child.
    child();
  default:
    // On success fork() returns the pid of the child to the parent.
    parent(pid);
  }
}



